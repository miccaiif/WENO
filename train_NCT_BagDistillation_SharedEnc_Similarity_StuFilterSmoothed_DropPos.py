...
class Optimizer:
    ...
    def optimize_teacher(self, epoch):
        ...
        feat_pred = torch.zeros([loader.dataset.__len__(), 100, 512]).float().to(self.dev)
...
if __name__ == "__main__":
    ...
    name = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")+"_%s" % args.comment.replace('/', '_') + \
           "_Seed{}_Bs{}_lr{}_BagLength{}_PosPatchRatio{}_Downsample{}_PLPostProcessBy{}_StuFilterType{}_smoothE{}_weightN{}_stuOptP{}".format(
               args.seed, args.batch_size, args.lr, args.bag_length, args.pos_patch_ratio, args.dataset_downsampling,
    ...
    writer = SummaryWriter('./runs_NCT/%s'%name)
    ...
    # Setup loaders
    positive_num = [2]
    negative_num = [0, 1, 3, 4, 5, 6, 7, 8]
    train_data, train_label, val_data, val_label = get_Path10_data(downsample_ratio=args.dataset_downsampling)

    train_ds_return_instance = NCT_WholeSlide_challenge(ds_data=train_data, ds_label=train_label, positive_num=positive_num, negative_num=negative_num,
    ...
    val_ds_return_instance = NCT_WholeSlide_challenge(ds_data=val_data, ds_label=val_label, positive_num=positive_num, negative_num=negative_num,
                                        bag_length=args.bag_length, return_bag=False,
                                        num_img_per_slide=args.bag_length, pos_patch_ratio=args.pos_patch_ratio, pos_slide_ratio=0.5, transform=None)
    val_ds_return_bag = NCT_WholeSlide_challenge(ds_data=val_data, ds_label=val_label, positive_num=positive_num, negative_num=negative_num,
